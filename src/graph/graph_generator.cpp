...
DynamicGraph GraphGenerator::generate_random_graph(
    ...
    mt19937_64 rng(seed);
    uniform_int_distribution<NodeId> node_dist(0, static_cast<NodeId>(num_nodes - 1));
    uniform_real_distribution<EdgeWeight> weight_dist(min_weight, max_weight);

    for (size_t i = 1; i < num_nodes; ++i) {
        NodeId parent = uniform_int_distribution<NodeId>(0, static_cast<NodeId>(i - 1))(rng);
        g.add_edge(parent, static_cast<NodeId>(i), weight_dist(rng));
    }
...
DynamicGraph GraphGenerator::generate_grid_graph(
    ...
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            NodeId u = static_cast<NodeId>(y * width + x);
            if (x + 1 < width) {
                NodeId v = static_cast<NodeId>(y * width + (x + 1));
                g.add_edge(u, v, weight_dist(rng));
            }
            if (y + 1 < height) {
                NodeId v = static_cast<NodeId>((y + 1) * width + x);
                g.add_edge(u, v, weight_dist(rng));
            }
        }
    }
    ...
    for (size_t u = 0; u < num_nodes; ++u) {
        for (size_t v = u + 1; v < num_nodes; ++v) {
            if (prob_dist(rng) < edge_prob) {
                g.add_edge(static_cast<NodeId>(u), static_cast<NodeId>(v), weight_dist(rng));
            }
        }
    }
...
DynamicGraph GraphGenerator::generate_flow_network(
    ...
    NodeId source = 0;
    NodeId sink = static_cast<NodeId>(total_nodes - 1);

    mt19937_64 rng(seed);
    uniform_int_distribution<FlowType> cap_dist(1, max_capacity);

    for (size_t i = 0; i < nodes_per_layer; ++i) {
        NodeId u = static_cast<NodeId>(1 + i);
        ...
        for (size_t i = 0; i < nodes_per_layer; ++i) {
            for (size_t j = 0; j < nodes_per_layer; ++j) {
                if (uniform_int_distribution<int>(0, 1)(rng)) {
                    g.add_flow_edge(
                        static_cast<NodeId>(l1_start + i),
                        static_cast<NodeId>(l2_start + j),
                        cap_dist(rng)
                    );
                }
            }
    ...
    for (size_t i = 0; i < nodes_per_layer; ++i) {
        NodeId u = static_cast<NodeId>(last_layer_start + i);
...
BitsetAdjacencyList GraphGenerator::generate_benchmark_bitset_graph(
    ...
    for (size_t i = 0; i < num_nodes; ++i) {
        NodeId next = static_cast<NodeId>((i + 1) % num_nodes);
        g.add_edge(static_cast<NodeId>(i), next, weight_dist(rng));
    }

    size_t extra_edges_per_node = (avg_degree > 1) ? (avg_degree - 1) : 1;
    for (size_t i = 0; i < num_nodes; ++i) {
        for (size_t d = 0; d < extra_edges_per_node; ++d) {
            NodeId target = node_dist(rng);
            if (target != i) {
                g.add_edge(static_cast<NodeId>(i), target, weight_dist(rng));
            }
        }
    }
